...
{
	sGame.m_InputManager.registerButton(sf::Mouse::Right);
}

GUIMarkingMenu::~GUIMarkingMenu(void)
{
	sGame.m_InputManager.unregisterButton(sf::Mouse::Right);
}


void GUIMarkingMenu::update(sf::Time _delta)
{
	if (!sGame.m_InputManager.getButtonState(sf::Mouse::Right) && sf::Mouse::isButtonPressed(sf::Mouse::Right))
	{
		//std::cout << "Right press" << std::endl;
		m_ActiveState = true;
		m_RawPoints.push_back(sf::Vector2f((float)(sf::Mouse::getPosition(sGame.m_Window).x), (float)(sf::Mouse::getPosition(sGame.m_Window).y)));
	}
	if (sGame.m_InputManager.getButtonState(sf::Mouse::Right) && !sf::Mouse::isButtonPressed(sf::Mouse::Right))
	{
	...
	if (m_ActiveState)
	{
		sf::Vector2f currentPoint = sf::Vector2f((float)(sf::Mouse::getPosition(sGame.m_Window).x), (float)(sf::Mouse::getPosition(sGame.m_Window).y));
