	...
	m_Unactivate(false)
{
	...
	if (m_DisplayText)
	{
		m_Text = sf::Text(_text, *sGame.m_FontManager.getFont(FontManager::FontID::OPENSANS_REGULAR), 20);
...
void GUIButton::update(sf::Time _delta)
{
	...
	if (m_Vertices.getBounds().contains(sf::Vector2f((float)(sf::Mouse::getPosition(sGame.m_Window).x), (float)(sf::Mouse::getPosition(sGame.m_Window).y))))
	{
		...
		else if (sGame.m_InputManager.getButtonState(sf::Mouse::Left) && !sf::Mouse::isButtonPressed(sf::Mouse::Left))
		{
