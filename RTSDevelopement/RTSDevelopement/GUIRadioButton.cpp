...
GUIRadioButton::GUIRadioButton(sf::Vector2f _position, std::string _text, bool _active/* = false*/) :
	...
	m_Description(_text, *sGame.m_FontManager.getFont(FontManager::FontID::OPENSANS_REGULAR), GUI_RADIO_CHAR_SIZE),
...
void GUIRadioButton::update(sf::Time _delta)
{
	...
	if (m_Outline.getBounds().contains(sf::Vector2f((float)(sf::Mouse::getPosition(sGame.m_Window).x), (float)(sf::Mouse::getPosition(sGame.m_Window).y))))
	{
