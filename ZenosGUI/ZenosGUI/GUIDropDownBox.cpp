...
GUIDropDownBox::GUIDropDownBox(sf::Vector2f _position, sf::Vector2f _size) :
	...
	m_SelectedOption("EMPTY BOX", *sGUIFONTS->getFont(FontManager::FontID::OPENSANS_REGULAR), (unsigned int)(_size.y - GUI_DROPDOWNBOX_CHAR_DIFF))
{
...
void GUIDropDownBox::update(sf::Time _delta)
{
	...
	if (m_ExtendedState == extendState::EXTENDED)
	{
		...
		for (unsigned int i = 0; i < m_ExtendedOptions.size(); i += 1)
		{
			if (m_ExtendedOptions.at(i)->getGlobalBounds().contains((float)(sf::Mouse::getPosition(*sGUIWINDOW).x), (float)(sf::Mouse::getPosition(*sGUIWINDOW).y)))
			{
...
{
	m_ExtendedOptions.push_back(new sf::Text(_option, *sGUIFONTS->getFont(FontManager::FontID::OPENSANS_REGULAR), (unsigned int)(m_Size.y - GUI_DROPDOWNBOX_CHAR_DIFF)));
