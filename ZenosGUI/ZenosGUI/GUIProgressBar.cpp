...
GUIProgressBar::GUIProgressBar(bool _horizontal, sf::Vector2f _position, sf::Vector2f _size) :
	m_Vertices(sf::Quads, 16),
	m_Size(_size),
	m_CompletedPercentage(0.0f),
	m_UnderwayPercentage(0.0f)
{
...
void GUIProgressBar::update(sf::Time _delta)
{
	...
	{
		if (m_Messages.front().s_MessageType == MessageData::MessageType::PROGRESSBAR_COMPLETE_PERC)
		{
			m_CompletedPercentage = m_Messages.front().s_FloatData;
		}
		else if (m_Messages.front().s_MessageType == MessageData::MessageType::PROGRESSBAR_UNDERWAY_PERC)
		{
			m_UnderwayPercentage = m_Messages.front().s_FloatData;
		}
