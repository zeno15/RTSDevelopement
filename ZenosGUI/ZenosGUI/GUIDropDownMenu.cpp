...
GUIDropDownMenu::GUIDropDownMenu(std::string _text, sf::Vector2f _position, float _length, bool _isRoot/* = false*/) :
	...
	m_Length(_length),
	m_Text(_text, *sGUIFONTS->getFont(FontManager::FontID::OPENSANS_REGULAR), GUI_DROPDOWNMENU_CHAR_SIZE)
{
...
{
	if (m_MenuBackground.getBounds().contains((float)(sf::Mouse::getPosition(*sGUIWINDOW).x), (float)(sf::Mouse::getPosition(*sGUIWINDOW).y)))
	{
